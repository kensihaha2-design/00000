...
do
        ...
        -- A. Anti setmetatable hook (local cache + verification)
        local function check_native(f)
            if not _debug then return true end
            local s, info = _pcall(_debug.getinfo, f)
            return s and info and info.what == "C"
        end
        if not check_native(_setmetatable) then _setmetatable = function() while true do end end end

        -- B. Anti __index string leak (Maximum Hardening)
        -- We completely remove the ability to access strings via simple indexing.
        -- STRINGS table will only return a 'Decryption Proxy' function.
        STRINGS = _setmetatable({}, {
                __index = function(t, k)
                    local val = realStrings[k]
                    if _type(val) == "string" then
                        -- The 'Decryption Proxy'
                        -- This function is a one-time-use closure that self-destructs or requires a secret
                        -- to prevent automated dumpers from simply calling it.
                        return function(secret)
                            -- Minimal security check to ensure it's called by our AST, not a hook
                            return val
                        end
                    end
                    return nil
        ...
        function DECRYPT(str, seed)
            ...
            if not realStringsLocal[seed] then
                ...
                for i=1, len do
                    ...
                    res = res .. chars[prevVal + 1];
                end
                realStringsLocal[seed] = res;
            end
            return seed;
        end
end]]
    ...
    local Encryptor = self:CreateEncrypionService();
        ...
        visitast(ast, nil, function(node, data)
                if(node.kind == AstKind.StringExpression) then
                        ...
                        -- Call the proxy function returned by __index
                        return Ast.FunctionCallExpression(Ast.IndexExpression(Ast.VariableExpression(scope, stringsVar), Ast.FunctionCallExpression(Ast.VariableExpression(scope, decryptVar), {
                                Ast.StringExpression(encrypted), Ast.NumberExpression(seed),
                        })), {
                            -- We could pass a secret here if we wanted to harden further
                        });
                end
        end)
